...
# Logging pre-configuration: like format, filename where logs should be created, level(info,warning etc)
logging.basicConfig(
...
Usage : Emulating the commands. Say, an attack logs into the honeypot and tries to run some commands.
...
'''
method : handle_connection(client, addr)
Usage :  Handling new connections to honeypot. 
         * sending SSH banner to client, so it feels attacker has the real server
         * Logging important metadata of client like, SSH client version, cipher, compression
'''
...
'''
method : Initiating the server
Usage : Open a port using python socket library then bind it to application.
        Python's threading -> create new thread for each new connection. for concurrent connection handling
'''
